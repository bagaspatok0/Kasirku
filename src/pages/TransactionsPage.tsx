...
import { transactionsService } from '@/lib/data-service';
...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    const unsubscribe = transactionsService.getAll(setTransactions);
    return () => unsubscribe();
  }, []);
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-4xl font-light tracking-tight text-zinc-900">Dashboard Penjualan</h1>
        <div className="flex flex-wrap items-center gap-2">
          <Button 
            variant="outline" 
            size="sm" 
            className="rounded-full bg-white border-zinc-100 text-zinc-600 hover:bg-zinc-50"
            onClick={setToday}
          >
            Hari Ini
          </Button>
          ...
          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-zinc-100 ml-2">
          ...
          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-zinc-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <Card className="rounded-3xl border-none shadow-sm bg-zinc-900 text-white">
          <CardContent className="p-6 md:p-8">
            <div className="flex items-center justify-between mb-4">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-white/10 rounded-2xl flex items-center justify-center">
                <TrendingUp className="w-5 h-5 md:w-6 md:h-6 text-zinc-100" />
              </div>
            </div>
            <p className="text-zinc-400 text-[10px] md:text-sm uppercase tracking-wider mb-1">Total Penjualan</p>
            <h2 className="text-2xl md:text-3xl font-medium tracking-tight">Rp {totalSales.toLocaleString()}</h2>
          </CardContent>
        </Card>
        <Card className="rounded-3xl border-none shadow-sm bg-white">
          <CardContent className="p-6 md:p-8">
            <div className="flex items-center justify-between mb-4">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-zinc-50 rounded-2xl flex items-center justify-center">
                <History className="w-5 h-5 md:w-6 md:h-6 text-zinc-900" />
              </div>
            </div>
            <p className="text-zinc-500 text-[10px] md:text-sm uppercase tracking-wider mb-1">Total Transaksi</p>
            <h2 className="text-2xl md:text-3xl font-medium tracking-tight text-zinc-900">{completedTransactions.length}</h2>
          </CardContent>
        </Card>
        <Card className="rounded-3xl border-none shadow-sm bg-white sm:col-span-2 lg:col-span-1">
          <CardContent className="p-6 md:p-8">
            <div className="flex items-center justify-between mb-4">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-zinc-50 rounded-2xl flex items-center justify-center">
            ...
            <p className="text-zinc-500 text-[10px] md:text-sm uppercase tracking-wider mb-1">Item Terjual</p>
            <h2 className="text-2xl md:text-3xl font-medium tracking-tight text-zinc-900">{totalItemsSold} <span className="text-sm md:text-lg font-light text-zinc-400">pcs</span></h2>
          </CardContent>
        </Card>
        <Card className="rounded-3xl border-none shadow-sm bg-white sm:col-span-2 lg:col-span-3">
          <CardContent className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4 md:gap-6">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-amber-50 rounded-2xl flex items-center justify-center shrink-0">
                <TrendingUp className="w-6 h-6 md:w-8 md:h-8 text-amber-500" />
              </div>
              <div className="min-w-0">
                <p className="text-zinc-500 text-[10px] md:text-sm uppercase tracking-wider mb-1">Item Terlaris</p>
                <h2 className="text-xl md:text-3xl font-medium tracking-tight text-zinc-900 truncate">
      ...
      <div className="bg-white rounded-[2rem] md:rounded-3xl border border-zinc-100 shadow-sm overflow-hidden p-4 md:p-6 lg:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
           <h3 className="text-xl md:text-2xl font-light tracking-tight px-1 text-center md:text-left">Riwayat Transaksi</h3>
           <div className="relative w-full md:w-80">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
              <Input 
                placeholder="Cari ID atau produk..." 
                className="pl-12 py-6 rounded-full bg-zinc-50 border-none outline-none focus:ring-1 focus:ring-zinc-200 transition-all text-sm shadow-inner"
                value={search}
        ...
        <div className="hidden md:block rounded-2xl border border-zinc-100 overflow-hidden">
          <Table>
            <TableHeader className="bg-zinc-50">
              <TableRow>
                <TableHead className="py-5 px-6">Waktu</TableHead>
            ...
            <TableBody>
              {filteredTransactions.map((t) => (
                ...
                >
                  <TableCell className="px-6 py-5 font-medium text-zinc-400 text-sm">
                    {t.createdAt?.toDate ? format(t.createdAt.toDate(), 'HH:mm, dd MMM', { locale: id }) : 'Recent'}
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-zinc-900 font-semibold">{t.customerName || '-'}</span>
                      <span className="text-[10px] text-zinc-400 font-mono tracking-tighter">ID: {t.id?.slice(-8)}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="max-w-xs space-y-1.5">
                      {t.items.map((i, idx) => (
                        <div key={idx} className="flex flex-col">
                          <span className="text-zinc-900 font-medium text-sm leading-tight">{i.name} <span className="text-zinc-400 text-xs">(x{i.quantity})</span></span>
                          {i.note && <span className="text-[10px] text-zinc-400 italic bg-zinc-50 w-fit px-1.5 py-0.5 rounded-md mt-0.5">"{i.note}"</span>}
                        </div>
                      ))}
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant={t.paymentMethod === 'cash' ? 'outline' : 'secondary'} className="rounded-full font-bold text-[10px] uppercase tracking-widest px-2.5 h-6">
                      {t.paymentMethod}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                       <Badge variant={t.status === 'void' ? 'destructive' : 'outline'} className="rounded-full font-bold text-[9px] px-2.5 h-6 uppercase tracking-widest">
                        {t.status === 'void' ? 'VOID' : 'COMPLETED'}
                      </Badge>
                      {t.isSettled && (
                        <Badge variant="secondary" className="rounded-full font-bold text-[9px] h-6 px-2.5 bg-emerald-50 text-emerald-600 border-emerald-100 uppercase tracking-widest">
                          SETTLED
                        </Badge>
                      )}
                    </div>
        ...
        <div className="md:hidden space-y-4">
          {filteredTransactions.map((t) => (
            <div 
              key={t.id} 
              className="bg-white border border-zinc-100 rounded-[2rem] p-6 shadow-sm active:scale-[0.98] transition-all"
              onClick={() => handleRowClick(t)}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">
                    {t.createdAt?.toDate ? format(t.createdAt.toDate(), 'HH:mm, dd MMM yyyy', { locale: id }) : 'Recent'}
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-zinc-900">{t.customerName || 'No Name'}</span>
                    <Badge variant={t.paymentMethod === 'cash' ? 'outline' : 'secondary'} className="rounded-full text-[8px] h-4 leading-none uppercase tracking-widest px-1.5 font-bold">
                      {t.paymentMethod}
                    </Badge>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                   <div className="flex gap-1.5 mb-1.5">
                      {t.isSettled && (
                        <Badge variant="secondary" className="rounded-full text-[8px] h-4 bg-emerald-50 text-emerald-600 border-none font-bold">SETTLED</Badge>
                      )}
                      <Badge variant={t.status === 'void' ? 'destructive' : 'outline'} className="rounded-full text-[8px] h-4 font-bold border-none uppercase tracking-widest">
                        {t.status === 'void' ? 'VOID' : 'OK'}
                      </Badge>
                   </div>
                   <span className={`text-lg font-black tracking-tight ${t.status === 'void' ? 'line-through text-zinc-200' : 'text-zinc-900'}`}>
                     Rp {t.totalAmount.toLocaleString()}
                   </span>
                </div>
              </div>

              <div className="bg-zinc-50 rounded-2xl p-4 space-y-2 mb-4">
                 {t.items.map((i, idx) => (
                   <div key={idx} className="flex justify-between items-center text-xs">
                     <span className="text-zinc-600 font-medium">{i.name} <span className="text-zinc-400 font-bold">x{i.quantity}</span></span>
              ...
              <div className="flex justify-between items-center gap-2">
                 <Button 
                    variant="secondary" 
                    className="flex-1 rounded-xl h-10 bg-zinc-50 text-zinc-600 font-bold text-[10px] uppercase tracking-widest hover:bg-zinc-100"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleRowClick(t);
                    }}
                  ...
                  {t.status !== 'void' && isCreatedToday(t.createdAt) && !t.isSettled && (
                    <Button 
                      variant="ghost" 
                      className="rounded-xl h-10 text-red-400 hover:text-red-500 font-bold text-[10px] uppercase tracking-widest px-4"
                      onClick={(e) => {
                        e.stopPropagation();
                        openVoidDialog(t);
                      }}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-zinc-900 uppercase tracking-wider">{selectedTransaction?.paymentMethod}</span>
                </div>
